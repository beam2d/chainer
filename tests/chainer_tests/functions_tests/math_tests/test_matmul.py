...
class TestMatMul(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ggx1 = numpy.random.uniform(-1, 1, self.x1_shape).astype(
            self.x1_dtype)
        self.ggx2 = numpy.random.uniform(-1, 1, self.x2_shape).astype(
            self.x2_dtype)
    ...
    def check_double_backward(self, x1_data, x2_data, y_grad,
                              x1_grad_grad, x2_grad_grad, atol, rtol):
    ...
    def test_matmul_double_backward_cpu(self):
        self.check_double_backward(self.x1, self.x2, self.gy, self.ggx1,
                                   self.ggx2, atol=1e-2, rtol=5e-2)

    @attr.gpu
    @condition.retry(3)
    def test_matmul_double_backward_gpu(self):
        self.check_double_backward(
            cuda.to_gpu(self.x1), cuda.to_gpu(self.x2), cuda.to_gpu(self.gy),
